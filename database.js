...
function buildLearningPath(lang) {
  const questions = getQuestions(lang, 1)
    .concat(getQuestions(lang, 2))
    .concat(getQuestions(lang, 3));
      ...
      skills: [
        {
          ...
          locked: false,
          mastery: 0,
          lessons: questions.filter((q) => Number(q.level) === 1).length,
        },
        {
          ...
          locked: false,
          mastery: 0,
          lessons: questions.filter((q) => Number(q.level) === 2).length,
        },
        {
          ...
          locked: false,
          mastery: 0,
          lessons: questions.filter((q) => Number(q.level) === 3).length,
        },
      ],
    },
  ];
